<div class="container container-small">
    <h2 class="h2-reason text-center">Reasons to use Morfy</h2>
    <div class="row">
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/magicwand.png" alt="">
                <h4>Simple</h4>
                <p>Easy to install, upgrade and use. Morfy installation only takes a few minutes.</p>
            </div>
        </div>
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/rocket.png" alt="">
                <h4>Fast &amp; light-weighted</h4>
                <p>Morfy is realy fast and lightweight!<br> Morfy doesn't use a database!</p>
            </div>
        </div>
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/gear.png" alt="">
                <h4>Flexible</h4>
                <p>Morfy provides basic api's for plugins, themes and core developers!</p>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/bookshelf.png" alt="">

                <h4>No database</h4>
                <p>No database required, flat files only! </p>
            </div>
        </div>
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/typography.png" alt="">

                <h4>Markdown formating</h4>
                <p>Edit your website in your favourite text editor using simple Markdown formatting.</p>
            </div>
        </div>
        <div class="col-md-4 wow fadeIn animated">
            <div class="features-columns">
                <img src="{$.config.site_url}/themes/default/assets/img/icons/merge.png" alt="">

                <h4>Open source</h4>
                <p>Morfy is an open-source project licensed under the GNU GENERAL PUBLIC LICENSE v3 to set the world free!</p>
            </div>
        </div>
    </div>
</div>
